  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section id="ranking" className="ranking-section"><div className="wrap"><div className="ranking-heading"><div><div className="eyebrow">まずは結論</div><h2>4年間の総費用目安ランキング</h2><p>学校納付金だけでなく、受験料・教材・住まい・生活費を同じ条件で試算。まず「どこが現実的か」をここで確認できます。</p></div><div className="ranking-mode"><span>生活スタイル</span><div className="toggle"><button className={livingMode === 'home' ? 'active' : ''} onClick={() => setLivingMode('home')}>自宅通学</button><button className={livingMode === 'away' ? 'active' : ''} onClick={() => setLivingMode('away')}>一人暮らし</button></div></div></div><div className="ranking-assumptions"><b>{livingMode === 'home' ? '自宅通学' : '一人暮らし'}の条件で比較中</b><span>{livingMode === 'home' ? '生活費 月2万円・住居費0円' : `生活費 月${monthlyLivingCostMan}万円・学校周辺の家賃目安`}</span><span>給付・減免：{annualScholarshipSupportMan > 0 ? `年${annualScholarshipSupportMan}万円を反映` : '未反映'}</span></div><div className="ranking-list">{costRankings.map((ranked) => <article className={`ranking-card ${ranked.rank === 1 ? 'featured' : ''}`} key={ranked.id}><div className="rank-num"><b>{ranked.rank}</b><small>位</small></div><div className="ranking-main"><div className="ranking-title-row"><div><h3>{ranked.name}</h3><p>{ranked.faculty}</p></div><span className="ranking-confidence">{ranked.confidence}</span></div><div className="ranking-cost"><span>4年間の総費用目安</span><b>{formatYen(ranked.cost.totalCostYen / 10000)}</b></div><div className="ranking-metrics"><span>学費 {formatYen(ranked.cost.schoolCostYen / 10000)}</span><span>住居 {formatYen(ranked.cost.housingCostYen / 10000)}</span><span>通学 {commuteDisplay(ranked)}</span><span>国試 {ranked.passRate}%</span></div><button className="ranking-detail" onClick={() => showCostForSchool(ranked.id)}>費用の内訳を見る →</button></div></article>)}</div><p className="ranking-note">※大学の優劣ではなく、現在収録している5校を共通条件で並べた費用目安です。未収録項目や推定値を含みます。支援制度は採用が確定した金額のみ反映してください。調査日：{datasetMeta.collectedAt.replaceAll('-', '.')}</p></div></section>
